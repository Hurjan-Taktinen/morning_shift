...
namespace mc
{
static const int ProtocolVersion = 404;
...
namespace ClientBound
{
    ...
    namespace Play
    {
        enum MessageId
        {
            ...
            // 0x03 	Spawn Mob
            // 0x04 	Spawn Painting
            SPAWN_PLAYER = 0x05, // 0x05 	Spawn Player
            ...
            // 0x0B 	Block Change
            // 0x0C 	Boss Bar
            SERVER_DIFFICULTY = 0x0D, // 0x0D 	Server Difficulty
            CHAT_MESSAGE = 0x0E, // 0x0E 	Chat Message (clientbound)
            // 0x0F 	Multi Block Change
            // 0x10 	Tab-Complete (clientbound)
            DECLARE_COMMANDS = 0x11, // 0x11 	Declare Commands
            // 0x12 	Confirm Transaction (clientbound)
            // 0x13 	Close Window (clientbound)
            WINDOW_ITEMS = 0x15, // 0x15 	Window Items
            // 0x16 	Window Property
            SET_SLOT = 0x17, // 0x17 	Set Slot
            // 0x18 	Set Cooldown
            PLUGIN_MESSAGE = 0x19, // 0x19 	Plugin Message (clientbound)
            // 0x1A 	Named Sound Effect
            DISCONNECT = 0x1B, // Disconnect (play)
            ENTITY_STATUS = 0x1C, // 0x1C 	Entity Status
            // 0x1E 	Explosion
            UNLOAD_CHUNK = 0x1F, // 0x1F 	Unload Chunk
            CHANGE_GAME_STATE = 0x20, // 0x20 	Change Game State
            KEEP_ALIVE = 0x21, // 0x21 	Keep Alive (clientbound)
            CHUNK_DATA = 0x22, // 0x22 	Chunk Data
            // 0x23 	Effect
            // 0x24 	Particle
            JOIN_GAME = 0x25,   // 0x25 	Join Game
            // 0x26 	Map Data
            ENTITY_RELATIVE_MOVEMENT = 0x28, // 0x28 	Entity Relative Move
            ENTITY_LOOK_AND_RELATIVE_MOVEMENT = 0x29,// 0x29 	Entity Look And Relative Move
            ENTITY_LOOK = 0x2A,// 0x2A 	Entity Look
            // 0x27 	Entity
            // 0x2B 	Vehicle Move (clientbound)
            // 0x14 	Open Window
            // 0x2C 	Open Sign Editor
            // 0x2D 	Craft Recipe Response
            PLAYER_ABILITIES = 0x2E, // 0x2E 	Player Abilities (clientbound)
            // 0x2F 	Combat Event
            PLAYER_INFO = 0x30, // 0x30 	Player Info
            // 0x31 	Face Player
            PLAYER_POSITION_AND_LOOK = 0x32, // 0x32 	Player Position And Look (clientbound)
            // 0x33     Use Bed
            UNLOCK_RECIPES = 0x34, // 0x34 	Unlock Recipes
            // 0x35 	Destroy Entities
            // 0x36 	Remove Entity Effect
            // 0x37 	Resource Pack Send
            // 0x38 	Respawn
            ENTITY_HEAD_LOOK = 0x39, // 0x39 	Entity Head Look
            ...
            // 0x47 	Teams
            // 0x48 	Update Score
            SPAWN_POSITION = 0x49, // 0x49 	Spawn Position
            TIME_UPDATE = 0x4A, // 0x4A 	Time Update
            // 0x4B 	Title
            ...
            // 0x4F 	Collect Item
            ENTITY_TELEPORT = 0x50, // 0x50 	Entity Teleport
...
namespace ServerBound
{
    ...
    namespace Play
    {
        enum MessageIds
        {
            ...
            CHAT_MESSAGE = 0x02, // 0x02 	Chat Message (serverbound)
            CLIENT_STATUS = 0x03, // 0x03 	Client Status
            CLIENT_SETTINGS = 0x04,         // 0x04 	Client Settings
            ...
            KEEP_ALIVE = 0x0E,              // 0x0E 	Keep Alive (serverbound)
            PLAYER_POSITION = 0x10,         // 0x10 	Player Position
            PLAYER_POSITION_AND_LOOK = 0x11,// 0x11 	Player Position And Look (serverbound)
            PLAYER_LOOK = 0x12, // 0x12 	Player Look
            PLAYER = 0x0F,                  // 0x0F 	Player
            // 0x13 	Vehicle Move (serverbound)
            // 0x14 	Steer Boat
            // 0x15 	Pick Item
            // 0x16 	Craft Recipe Request
            // 0x17 	Player Abilities (serverbound)
            PLAYER_DIGGING = 0x18, // 0x18 	Player Digging
            // 0x19 	Entity Action
            // 0x1A 	Steer Vehicle
